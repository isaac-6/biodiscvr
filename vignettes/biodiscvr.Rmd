...
1.  **Root Data Directory:** The main folder containing subdirectories for each dataset (e.g., "ADNI", "OASIS"). Each subdirectory should contain at least `data.csv` and `data_suv_bi.csv`. The file `data_vol_bi` will be used for volume-weighted combinations from the literature.

The following are provided by the package, for this project1.
...
You will need to decide on output locations:
...
*   `literature_biomarkers.yaml`: Default set of literature biomarkers.
...
user_supporting_files_dir <- NULL # <<< REPLACE IF USING CUSTOM CONFIG/DICT
                                                          # OR set to NULL to use package defaults

# 3. Path to your CUSTOM experiments definition file (if not using package default)
user_experiments_file <- system.file("files", "experiments.yaml", package = "biodiscvr") # <<< REPLACE IF CUSTOM
...
user_literature_defs_file <- system.file("files", "literature_biomarkers.yaml", package = "biodiscvr") # <<< REPLACE IF CUSTOM
...
user_log_dir <- file.path(user_output_dir, ".log")
...
*   Calculating the `time` variable from a date column (e.g., `ScanDate` or `years.from.baseline`).
...
## Step 5: Evaluating Discovered Biomarkers

After running experiments, you will have a CSV file (`main_results_csv`) containing various discovered biomarkers (e.g., from baseline runs, fixed region runs, multi-cohort runs). This step uses the `evaluate_biomarkers` function to take one or more of these discovered biomarkers (identified by their `regs_numerator` and `regs_denominator` from the CSV) and evaluate their performance (Rep, SepAB, SSE) across all your preprocessed datasets and for both CU and CI groups.

**Note:** This chunk is set to `eval=FALSE`. You'll need to ensure `main_results_csv` exists from Step 4.

```{r evaluate_discovered_user, eval=FALSE}
# Ensure main_results_csv, preprocessed_data_list, and config_loaded are available
main_results_csv_path <- file.path(user_output_dir, main_results_csv)

# Define path for the evaluation output
evaluation_output_csv <- file.path(user_output_dir, "biomarker_evaluations.csv")

if (file.exists(main_results_csv_path)) {
  message("Evaluating discovered biomarkers from: ", main_results_csv)
  
  # For this example, we assume evaluate_biomarkers will process the whole file.

  evaluated_biomarkers_df <- evaluate_biomarkers(
    discovery_results_csv_path = main_results_csv_path,
    prepared_data_list = preprocessed_data_list,
    config = config_loaded,
    datasets_to_evaluate = names(preprocessed_data_list), # Evaluate on all datasets
    groups_to_evaluate = c("CU", "CI"), # Evaluate for both groups
    calculate_ci = TRUE, # Set to TRUE for 95% CIs (slower)
    nsim = 2000, # If calculate_ci = TRUE
    output_evaluation_csv_path = evaluation_output_csv,
    verbose = TRUE
  )
  
  if(!is.null(evaluated_biomarkers_df) && nrow(evaluated_biomarkers_df) > 0){
    message("Evaluation of discovered biomarkers complete. Results saved to: ", evaluation_output_csv)
    # print(head(evaluated_biomarkers_df))
  } else {
    message("Evaluation of discovered biomarkers did not produce results or the input CSV was empty/problematic.")
  }
} else {
  message("Skipping evaluation of discovered biomarkers: Main results CSV '", 
          basename(main_results_csv), "' not found.")
  message("Ensure the 'Running Discovery Experiments' step (Step 4) ran successfully and created the CSV.")
}
```

*Expected Output (when run manually):* A new CSV file (`final_biomarker_evaluations.csv`) containing detailed Rep, SepAB, and SSE metrics for each selected discovered biomarker, evaluated on each specified dataset and group.

## 6. Iterative Regional Ablation

Perform ablation on the best biomarker found from the evaluation file or experiments run.
This code finds a better-performing subset of regions from the provided biomarkers, if it exists.

```{r run_ablation_synth_v3, eval=FALSE}
# Define path for the evaluation output
ablation_output_csv <- file.path(user_output_dir, "biomarker_ablation.csv")

if (!is.null(evaluation_output_csv)) {
  message("Performing iterative ablation on the discovered biomarkers...")

    run_ablation(
        discovered_biomarkers_csv_path = evaluation_output_csv,
        prepared_data_list = preprocessed_data_list,
        config = config_loaded,
        output_ablation_results_csv_path = ablation_output_csv,
        datasets_for_ablation_eval = NULL, # NULL uses the same dataset as the row in the file
        groups_for_ablation_eval = NULL, # NULL uses the same group as the row in the file
        features_for_ablation = NULL, # NULL uses available biomarker features.
        verbose = TRUE) 
...
## Step 7: Evaluating Literature Biomarkers

Evaluates biomarkers from your `literature_biomarkers.yaml` (custom or package default).

```{r eval_literature, eval=FALSE}
if (file.exists(user_literature_defs_file)) {
  literature_file_to_use <- user_literature_defs_file
} else {
  literature_file_to_use <- pkg_literature_defs_file
}
if (!file.exists(literature_file_to_use)) {
  warning("Literature biomarker file not found. Skipping evaluation.", immediate.=TRUE)
} else {
  evaluate_literature_biomarkers(lit_biomarker_file_path = literature_file_to_use,
                               prepared_data_list = preprocessed_data_list,
                               config = config_loaded,
                               ...
                               calculate_ci = TRUE,
                               nsim = 2000,
                               output_csv_path = literature_results_csv,
                               id_col = NULL, # NULL considers "RID" as the unique individual identifier
                               verbose = TRUE) 
...
1.  **Main Discovery & Evaluation Results:**
    ...
    *   Content: Results from `run_experiments`, `evaluate_biomarkers` and `run_ablation``.

2.  **Literature Biomarker Evaluation Results:**
    *   Path: `user_output_dir`
    *   Content: Performance metrics for literature biomarkers.
