...
#'   is `TRUE`. Defaults to 100. Be mindful that higher values increase runtime significantly.
#'
#' @return A named numeric vector containing the evaluation metrics.
#'   If `calculate_ci` is `FALSE`: `eRepCI`, `eRepCU`, `eSep`, `nCI`, `nCU`.
#'   If `calculate_ci` is `TRUE`: `eRepCI`, `eRepCI.lower`, `eRepCI.upper`,
#'   `eRepCU`, `eRepCU.lower`, `eRepCU.upper`, `eSep`, `eSep.lower`, `eSep.upper`,
#'   `nCI`, `nCI.lower`, `nCI.upper`, `nCU`, `nCU.lower`, `nCU.upper`.
#'   Values may be `NA` if models fail to converge or required data subsets are empty.
#'
#' @details
#' This function wraps internal helpers to perform the following steps:
#' 1. Fits models using `lme4::lmer` based on the provided formulas (`eq_all`, `eq_group`)
#'    and subsets of the data (Amyloid positive CU and CI groups).
#' 2. Extracts repeatability (`sd(residuals)`) and separation (`t-value` of `time:DX`).
#' 3. Estimates sample sizes using `longpower::lmmpower`.
#' 4. Optionally performs parametric bootstrapping using `lme4::bootMer` to
#'    estimate 95% confidence intervals for repeatability and separation.
#'
#' Note: Ensure the input `data` is appropriately prepared (e.g., scaling time,
#' handling missing values if necessary before calling this function). Model
#' convergence is attempted using `nloptwrap` optimizer settings. Warnings
...
fevalMethod <- function(data, group, eq_all, eq_group, all_power_params, lmer_control, calculate_ci = FALSE, nsim = 1000) {
  ...
  if (!inherits(eq_all, "formula")) stop("'eq_all' must be a formula.")
  if (!inherits(eq_group, "formula")) stop("'eq_group' must be a formula.")
  if (!is.list(power_params)) stop("'power_params' must be a list.")
  if (!is.logical(calculate_ci) || length(calculate_ci) != 1) stop("'calculate_ci' must be TRUE or FALSE.")
  if (!is.numeric(nsim) || length(nsim) != 1 || nsim <= 1) stop("'nsim' must be a number greater than 1.")
  
  
  if (calculate_ci) {
    # Call internal function with 95%CI calculation
    metrics <- .fevalMethod_dCI(data = data,
                                group = group,
                                eq_all = eq_all,
                                eq_group = eq_group,
                                all_power_params = all_power_params, 
                                lmer_control = lmer_control,
                                nsim = as.integer(nsim)) # Ensure integer nsim
  } else {
    # Call internal function without 95%CI calculation
    metrics <- .fevalMethod_d(data = data,
                              group = group,
                              eq_all = eq_all,
                              eq_group = eq_group,
                              all_power_params = all_power_params, 
